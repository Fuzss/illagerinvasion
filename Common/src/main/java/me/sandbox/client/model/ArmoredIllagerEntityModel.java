...
import me.sandbox.entity.monster.Inquisitor;
...
public class ArmoredIllagerEntityModel<T extends AbstractIllager> extends HierarchicalModel<T> implements ArmedModel, HeadedModel {
    ...
    public static LayerDefinition getTexturedModelData() {
        ...
        modelPartData.addOrReplaceChild("left_shoulderpad", CubeListBuilder.create().texOffs(0, 64).mirror().addBox(-0.7f, -2.0f, -2.0f, 4.0f, 4.0f, 4.0f, new CubeDeformation(1.1f)), PartPose.offset(5.0f, 2.0f, 0.0f));
        modelPartData.addOrReplaceChild("right_shoulderpad", CubeListBuilder.create().texOffs(0, 64).addBox(-3.3f, -2.0f, -2.0f, 4.0f, 4.0f, 4.0f, new CubeDeformation(1.1f)), PartPose.offset(-5.0f, 2.0f, 0.0f));
    ...
    public void setupAnim(T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (this.riding) {
            ...
            this.rightShoulderpad.xRot = -0.62831855f;
            this.leftShoulderpad.xRot = -0.62831855f;
        } else {
            this.rightArm.xRot = Mth.cos(limbSwing * 0.6662f + 3.1415927f) * 2.0f * limbSwingAmount * 0.5f;
            this.rightShoulderpad.xRot = Mth.cos(limbSwing * 0.6662f + 3.1415927f) * 2.0f * limbSwingAmount * 0.5f;
            this.rightShoulderpad.yRot = 0.0f;
            this.rightArm.yRot = 0.0f;
            this.rightArm.zRot = 0.0f;
            this.leftArm.xRot = Mth.cos(limbSwing * 0.6662f) * 2.0f * limbSwingAmount * 0.5f;
            this.leftShoulderpad.xRot = Mth.cos(limbSwing * 0.6662f) * 2.0f * limbSwingAmount * 0.5f;
            this.leftShoulderpad.yRot = 0.0f;
            this.leftArm.yRot = 0.0f;
            this.leftArm.zRot = 0.0f;
        ...
        final boolean hasShield = entity.getOffhandItem().is(Items.SHIELD);
        if (state == AbstractIllager.IllagerArmPose.ATTACKING) {
            if (((LivingEntity) entity).getMainHandItem().isEmpty()) {
                ...
                AnimationUtils.animateZombieArms(this.leftShoulderpad, this.rightShoulderpad, true, this.attackTime, ageInTicks);
            } else {
                AnimationUtils.swingWeaponDown(this.rightArm, this.leftArm, (Mob) entity, this.attackTime, ageInTicks);
                AnimationUtils.swingWeaponDown(this.rightShoulderpad, this.leftShoulderpad, (Mob) entity, this.attackTime, ageInTicks);
            }
            if (hasShield && mainArm == HumanoidArm.RIGHT) {
                this.leftArm.xRot = -1.05f;
                this.leftArm.yRot = 0.5235988f;
                this.leftShoulderpad.xRot = -1.05f;
                this.leftShoulderpad.yRot = 0.5235988f;
            }
            if (hasShield && mainArm == HumanoidArm.LEFT) {
                this.rightArm.xRot = 200.0f;
                this.rightArm.yRot = -0.5235988f;
                this.rightShoulderpad.xRot = 200.0f;
                this.rightShoulderpad.yRot = -0.5235988f;
            }
            if (((Inquisitor) entity).getStunnedState()) {
                this.head.xRot = 18.5f;
                ...
                this.leftArm.yRot = -0.5235988f;
                this.leftShoulderpad.xRot = 200.0f;
                this.leftShoulderpad.yRot = -0.5235988f;
            }
            ...
            this.rightArm.yRot = 0.0f;
            this.rightShoulderpad.z = 0.0f;
            this.rightShoulderpad.x = -5.0f;
            this.rightShoulderpad.xRot = Mth.cos(ageInTicks * 0.6662f) * 0.05f;
            this.rightShoulderpad.zRot = 2.670354f;
            this.rightShoulderpad.yRot = 0.0f;
            this.leftArm.z = 0.0f;
            this.leftArm.x = 5.0f;
            ...
            this.leftArm.zRot = -2.3561945f;
            this.leftArm.yRot = 0.0f;
            this.leftShoulderpad.z = 0.0f;
            this.leftShoulderpad.x = 5.0f;
            this.leftShoulderpad.xRot = Mth.cos(ageInTicks * 0.6662f) * 0.05f;
            this.leftShoulderpad.zRot = -2.3561945f;
            this.leftShoulderpad.yRot = 0.0f;
        }
