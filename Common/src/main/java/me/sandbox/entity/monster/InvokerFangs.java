...
public class InvokerFangs extends Entity {
    ...
    public void handleEntityEvent(byte status) {
        ...
        if (status == 4) {
            ...
            if (!this.isSilent()) {
                this.level().playLocalSound(this.getX(), this.getY(), this.getZ(), ModRegistry.INVOKER_FANGS_SOUND_EVENT, this.getSoundSource(), 1.0f, this.random.nextFloat() * 0.2f + 0.85f, false);
            }
        }
    }
