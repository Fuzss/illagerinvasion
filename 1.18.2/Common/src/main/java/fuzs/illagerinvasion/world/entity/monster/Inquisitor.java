...
public class Inquisitor extends AbstractIllager {
    ...
    @Override
    protected void populateDefaultEquipmentSlots(final DifficultyInstance difficulty) {
    ...
    public boolean hurt(final DamageSource source, final float amount) {
        ...
        if (this.isAggressive()) {
            if (attacker instanceof LivingEntity) {
                ...
                if ((ToolTypeHelper.INSTANCE.isAxe(item) || attacker instanceof IronGolem || this.blockedCount >= 4) && isShield) {
    ...
    static class NodeEvaluator extends WalkNodeEvaluator {
        ...
        protected BlockPathTypes evaluateBlockPathType(BlockGetter level, boolean canOpenDoors, boolean canEnterDoors, BlockPos pos, BlockPathTypes nodeType) {
            return nodeType == BlockPathTypes.LEAVES ? BlockPathTypes.OPEN : super.evaluateBlockPathType(level, canOpenDoors, canEnterDoors, pos, nodeType);
        }
    }
